...
/*
	timestamp += __x;  \
	if (!overclocked) soundtimestamp +=  __x; \
*/
	...
	while (_count > 0) {
		...
		if (_IRQlow) {
			if (_IRQlow & FCEU_IQRESET) {
				...
				_jammed = 0;
				_PI = _P = I_FLAG;
				_IRQlow &= ~(FCEU_IQRESET|FCEU_IQTEMP);
			} else if (_IRQlow & FCEU_IQNMI2) {
				_IRQlow &= ~(FCEU_IQNMI2|FCEU_IQTEMP);
				_IRQlow |= FCEU_IQNMI;
			...
			if (_count <= 0) {
				_PI = _P;
				X.tsp.val+=((_tcount-bakcnt)<<16)|(_tcount-bakcnt);
				return;
